...
The parent skill picks which PE(s) to dispatch from the project's Stack Map — `.code-reviewer.yml` first, then the `## Stack Map` table in `AGENTS.md` or `CLAUDE.md` — generated by `/init-project`. When a repo has both instruction files, `CLAUDE.md` is read first unless it only imports `@AGENTS.md`.
