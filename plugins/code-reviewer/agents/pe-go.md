...
Go-specific examples for `silence_check`:
- timeouts defaulting to `context.Background()` (no deadline propagation)
- error wrapping with `fmt.Errorf("...%v", err)` (loses chain — use `%w`)
